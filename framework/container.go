...
// CustomContainer 是服务容器的具体实现
type CustomContainer struct {
	// 组合一个Container接口, 该struct必须实现Container的所有接口
	Container
...
func (c *CustomContainer) PrintProviders() []string {
	ret := []string{}
...
// Bind 将服务容器和关键字做了绑定
func (c *CustomContainer) Bind(provider ServiceProvider) error {
	c.lock.Lock()
	key := provider.Name()
...
func (c *CustomContainer) IsBind(key string) bool {
	return c.findServiceProvider(key) != nil
}

func (c *CustomContainer) findServiceProvider(key string) ServiceProvider {
	c.lock.RLock()
...
func (c *CustomContainer) Make(key string) (interface{}, error) {
	return c.make(key, nil, false)
}

func (c *CustomContainer) MustMake(key string) interface{} {
...
func (c *CustomContainer) MakeNew(key string, params []interface{}) (interface{}, error) {
	return c.make(key, params, true)
}

func (c *CustomContainer) newInstance(sp ServiceProvider, params []interface{}) (interface{}, error) {
	// force new a
...
// 真正的实例化一个服务
func (c *CustomContainer) make(key string, params []interface{}, forceNew bool) (interface{}, error) {
	c.lock.RLock()
	defer c.lock.RUnlock()
...
func (c *CustomContainer) NameList() []string {
	ret := []string{}
	for _, provider := range c.providers {
		name := provider.Name()
		ret = append(ret, name)
