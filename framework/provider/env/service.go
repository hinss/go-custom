...
// CustomEnv 是 Env 的具体实现
type CustomEnv struct {
	folder string            // 代表.env所在的目录
	maps   map[string]string // 保存所有的环境变量
}

// NewCustomEnv 有一个参数，.env文件所在的目录
// example: NewCustomEnv("/envfolder/") 会读取文件: /envfolder/.env
// .env的文件格式 FOO_ENV=BAR
func NewCustomEnv(params ...interface{}) (interface{}, error) {
	if len(params) != 1 {
		return nil, errors.New("NewCustomEnv param error")
	}
...
func (en *CustomEnv) AppEnv() string {
	return en.Get("APP_ENV")
}

// IsExist 判断一个环境变量是否有被设置
func (en *CustomEnv) IsExist(key string) bool {
	_, ok := en.maps[key]
	return ok
}

// Get 获取某个环境变量，如果没有设置，返回""
func (en *CustomEnv) Get(key string) string {
...
func (en *CustomEnv) All() map[string]string {
	return en.maps
}
