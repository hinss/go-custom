...
// CustomConfig  表示custom框架的配置文件服务
...
// 读取某个配置文件
func (conf *CustomConfig) loadConfigFile(folder string, file string) error {
	conf.lock.Lock()
...
// 删除文件的操作
func (conf *CustomConfig) removeConfigFile(folder string, file string) error {
	conf.lock.Lock()
	defer conf.lock.Unlock()
	s := strings.Split(file, ".")
	// 只有yaml或者yml后缀才执行
...
// NewCustomConfig 初始化Config方法
func NewCustomConfig(params ...interface{}) (interface{}, error) {
...
// 通过path来获取某个配置项
func (conf *CustomConfig) find(key string) interface{} {
	conf.lock.RLock()
