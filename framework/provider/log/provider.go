...
// CustomLogServiceProvider 服务提供者
type CustomLogServiceProvider struct {
...
// Register 注册一个服务实例
func (l *CustomLogServiceProvider) Register(c framework.Container) framework.NewInstance {
	if l.Driver == "" {
		tcs, err := c.Make(contract.ConfigKey)
		if err != nil {
			// 默认使用console
			return services.NewCustomConsoleLog
		}
	...
	switch l.Driver {
	case "single":
		return services.NewCustomSingleLog
	case "rotate":
		return services.NewCustomRotateLog
	case "console":
		return services.NewCustomConsoleLog
	case "custom":
		return services.NewCustomCustomLog
	default:
		return services.NewCustomConsoleLog
	}
}
...
// Boot 启动的时候注入
func (l *CustomLogServiceProvider) Boot(c framework.Container) error {
	return nil
}

// IsDefer 是否延迟加载
func (l *CustomLogServiceProvider) IsDefer() bool {
	return false
}

// Params 定义要传递给实例化方法的参数
func (l *CustomLogServiceProvider) Params(c framework.Container) []interface{} {
	// 获取configService
...
// Name 定义对应的服务字符串凭证
func (l *CustomLogServiceProvider) Name() string {
