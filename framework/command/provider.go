...
var providerListCommand = &cobra.Command{
	...
	RunE: func(c *cobra.Command, args []string) error {
		...
		customContainer := container.(*framework.CustomContainer)
		// 获取字符串凭证
		list := customContainer.NameList()
		// 打印
...
var providerCreateCommand = &cobra.Command{
	...
	RunE: func(c *cobra.Command, args []string) error {
		...
		// 检查服务是否存在
		providers := container.(*framework.CustomContainer).NameList()
...
import "github.com/hinss/go-custom/framework"
