...
type devConfig struct {
	...
	Host    string   // 调试模式最终监听ip 默认0.0.0.0
...
func (p *Proxy) rebuildBackend() error {
	// 重新编译hade
	cmdBuild := exec.Command("./notes", "build", "backend")
...
func (p *Proxy)restartBackend() error {
	...
	// 使用命令行启动后端进程
	cmd := exec.Command("./notes", "app", "start", "--address="+hadeAddress)
...
func (p *Proxy) startProxy(startFrontend, startBackend bool) error {
	...
	proxyServer := &http.Server{
		Addr:    p.devConfig.Host + ":" + p.devConfig.Port,
