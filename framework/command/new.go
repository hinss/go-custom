...
var newCommand = &cobra.Command{
	...
	RunE: func(c *cobra.Command, args []string) error {
		...
		}
		{
			...
			prompt := &survey.Input{
				Message: "请输入版本名称(参考 https://github.com/hinss/go-custom/releases，默认为最新版本)：",
			}
		...
		fmt.Println("custom框架版本：", release.GetTagName())

		templateFolder := filepath.Join(currentPath, "template-custom-"+version+"-"+cast.ToString(time.Now().Unix()))
		...
		// 获取folder下的gocustom-custom-xxx相关解压目录
		...
		filepath.Walk(folder, func(path string, info os.FileInfo, err error) error {
			...
			if path == filepath.Join(folder, "go.mod") {
				...
				c = bytes.ReplaceAll(c, []byte("module github.com/hinss/go-custom"), []byte("module "+mod))
				c = bytes.ReplaceAll(c, []byte("require ("), []byte("require (\n\tgithub.com/hinss/go-custom "+version))
			...
			isContain := bytes.Contains(c, []byte("github.com/hinss/go-custom"))
			if isContain {
				fmt.Println("更新文件:" + path)
				c = bytes.ReplaceAll(c, []byte("github.com/hinss/go-custom"), []byte(mod))
				err = ioutil.WriteFile(path, c, 0644)
				if err != nil {
					return err
				}
